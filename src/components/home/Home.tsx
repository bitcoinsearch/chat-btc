...
const HomePage = ({ onPrompt }: HomePageProps) => {
  ...
  // const currentSegment = useScrollEffectContainer({ containerRef: fullScreenContainer });
  ...
  return (
    <>
      ...
      >
        <InView threshold={0.5} onChange={(inView, entry) => handleInView(inView, entry.target.id)}>
          {({ref}) => (
          <Box
            id="chat"
            pt={{ base: 4, md: 10 }}
        ...
        <InView threshold={0.5} onChange={(inView, entry) => handleInView(inView, entry.target.id)}>
          {({ref}) => (
          <Box id="about" py={10} display="flex" alignItems={"center"} ref={ref}>
            <Container maxW="2xl">
...
const InLink = ({ name, currentSegment }: { name: SectionName, currentSegment: string }) => {
